...
- `base.py` This module contains `BaseInstaller` class which is inherited by all pther installers. It provides a skeleton and 
  ...
  methods/fucntions automatically:
   ...
   - `download_files()`: if war files to be downloaded, downloads from `source_files` defined in installer, you can implement this function within installer, see for example `casa.py`
