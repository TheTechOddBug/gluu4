...
This documentation demonstrates how to upgrade a Kubernetes setup of Gluu >=4.2 multi-region LDAP/Opendj to 4.5 PostgreSQL.

!!! Note
    Custom scripts will need manual intervention due to the huge gap between Gluu versions.
...
1.  Edit the manifest of the current OpenDJ statefulset:
    ...
    Upgrade the image tag to `4.5.15-x` and add a new env variable:
    ...
    #      name: my-custom-schema
    containers:
      - image: gluufederation/opendj:4.5.15-1
        env:
    ...
    spec:
      template:
        ...
        spec:
          ...
          containers:
            - name: upgrade-42
              image: gluufederation/upgrade:4.5.15-1
              volumeMounts: []
              envFrom:
                - configMapRef:
...
1.  Prepare Postgres database for migration. You should have a production-ready Postgres database. For testing purposes:

    ```bash
    kubectl create ns gluu
    https://raw.githubusercontent.com/GluuFederation/gluu4/refs/heads/4.5/cloud-native-edition/automation/pgsql.yaml
    kubectl apply -f pgsql.yaml # Modify as needed
    ```
        ...
        spec:
          template:
            ...
            spec:
              ...
              containers:
                - name: offline-persistence-load
                  image: gluufederation/persistence:4.5.15-1
                  volumeMounts:
                    - name: custom-gluu-ldif
...
1.  Jackrabbit is deprecated in Gluu 4.5. You can follow this [guide](https://docs-4.gluu.org/gluu-server/installation-guide/install-kubernetes/#migrating-from-jackrabbit) to migrate the document store type from Jackrabbit to DB. Also, follow this [guide](https://docs-4.gluu.org/gluu-server/operation/custom-script-customization) to demonstrate how to use ConfigMaps and custom scripts for custom pages instead of Jackrabbit.
