...
1.  Change the ownership of the Opendj filesystem: 
    ...
    Configmaps are mounted as read-only files.
    So, if you have any configmap mounted under `/opt/opendj`, you'll need to add their mount path so that it's ingnored by the `chown` command:

    ```
    kubectl exec <opendj-pod-name> -n <namespace> -- -- sh -c \
    > 'find /opt/opendj -path "/opt/opendj/template/config/schema/ceva-customAttributes.ldif" -prune -o -exec chown 1000:root {} +'
    ```
