...
public class OIDCService {
    ...
    public Pair<String, String> getAuthnRequestUrl(List<String> acrValues, String prompt) throws GeneralException {
        ...
        try {
            ClientID clientID = new ClientID(settings.getClientId());
    ...
    public Pair<String, String> getTokens(String code) throws GeneralException {
        ...
        try {
            ...
            ClientID clientID = new ClientID(settings.getClientId());
            Secret clientSecret = new Secret(settings.getClientSecret());
    ...
    public Map<String, Object> getUserClaims(String accessToken) throws GeneralException {
        ...
        try {
            ...
            if (!userInfoResponse.indicatesSuccess()) {
                ...
                if (successResponse.getEntityContentType().equals(APPLICATION_JSON)) {
                    userInfo = new UserInfo(successResponse.toHTTPResponse().getBodyAsJSONObject());
                } else {
                    userInfo = new UserInfo(successResponse.getUserInfoJWT().getJWTClaimsSet());
                }
       
                claims = userInfo.toJSONObject().entrySet().stream()
